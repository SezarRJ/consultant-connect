 ...
 * Contacts, lead pipeline, qualification, Convert to Engagement
 */
...
import {
  ...
  ChevronRight, DollarSign, Calendar, Tag, ArrowRight,
  CheckCircle2, AlertTriangle, Clock, Briefcase, Star, Trash2
} from "lucide-react";
...
function ContactCard({ c, onClick, selected }: { c:CRMContact; onClick:()=>void; selected:boolean }) {
  ...
  return (
    <button onClick={onClick}
      className="w-full text-left p-4 rounded-xl transition-all hover:opacity-90 group"
      style={{
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl p-5" style={{ background:"hsl(216 45% 10%)", border:"1px solid hsl(216 45% 18%)" }}>
        <div className="flex items-start justify-between gap-3 mb-4">
          ...
          <div className="flex gap-2">
            <button onClick={onEdit} className="text-xs px-3 py-1.5 rounded-lg" style={{ background:"hsl(216 45% 18%)", color:"hsl(215 25% 60%)" }}>Edit</button>
            <button onClick={onDelete} className="p-1.5 rounded-lg" style={{ background:"hsl(0 72% 51%/0.12)", color:"hsl(0 72% 60%)" }}><Trash2 className="h-3.5 w-3.5" /></button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            <div key={i} className="flex items-center gap-2">
              ...
              <span className="text-xs truncate" style={{ color:"hsl(210 40% 75%)" }}>{val as string}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {c.qualificationStatus === "Qualified" && (
        <div className="rounded-xl p-4 space-y-2" style={{ background:"hsl(145 65% 40%/0.06)", border:"1px solid hsl(145 65% 40%/0.2)" }}>
          ...
          {c.qualificationNotes && <p className="text-xs" style={{ color:"hsl(145 65% 45%)" }}>{c.qualificationNotes}</p>}
        </div>
      )}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background:"rgba(0,0,0,0.7)" }}>
      <div className="w-full max-w-2xl rounded-xl p-6 space-y-4 overflow-y-auto max-h-[90vh]"
        ...
        <div className="grid grid-cols-2 gap-3">
          {([
            ["Full Name","fullName","text"],["Job Title","jobTitle","text"],
            ["Email","email","email"],["Phone","phone","text"],
            ["Company","companyName","text"],["Sector","sector","text"],
          ...
          {([
            ["Country","country",COUNTRIES],["Industry","industry",INDUSTRIES],
            ["Lead Source","leadSource",SOURCES],["Service Interest","interestedService",SERVICES],
            ["Urgency","urgency",["High","Medium","Low"]],["Priority","priorityLevel",["High","Medium","Low"]],
            ["Lead Status","leadStatus",STATUSES],["Qualification","qualificationStatus",["Pending","Qualified","Rejected"]],
          ] as [string,string,string[]][]).map(([label,key,opts]) => (
            <div key={key}>
              <label className="block text-[10px] font-semibold uppercase tracking-wider mb-1" style={{ color:"hsl(215 25% 48%)" }}>{label}</label>
              <select value={(form as any)[key]} onChange={f(key)} className="w-full rounded-md px-3 py-1.5 text-sm"
                style={{ background:"hsl(216 45% 12%)", color:"hsl(210 40% 88%)", border:"1px solid hsl(216 45% 22%)" }}>
          ...
          <div>
            <label className="block text-[10px] font-semibold uppercase tracking-wider mb-1" style={{ color:"hsl(215 25% 48%)" }}>Estimated Budget</label>
            <input type="text" value={form.estimatedBudget} onChange={f("estimatedBudget")} placeholder="e.g. $42,000"
              className="w-full rounded-md px-3 py-1.5 text-sm"
              style={{ background:"hsl(216 45% 12%)", color:"hsl(210 40% 88%)", border:"1px solid hsl(216 45% 22%)" }} />
          </div>
          <div>
            <label className="block text-[10px] font-semibold uppercase tracking-wider mb-1" style={{ color:"hsl(215 25% 48%)" }}>Next Action Date</label>
            <input type="date" value={form.nextActionDate} onChange={f("nextActionDate")}
              className="w-full rounded-md px-3 py-1.5 text-sm"
              style={{ background:"hsl(216 45% 12%)", color:"hsl(210 40% 88%)", border:"1px solid hsl(216 45% 22%)" }} />
          </div>
        </div>
        {([["Client Need","clientNeed"],["Business Problem","businessProblem"],["Qualification Notes","qualificationNotes"],["Notes","notes"]] as [string,string][]).map(([l,k]) => (
          <div key={k}>
            <label className="block text-[10px] font-semibold uppercase tracking-wider mb-1" style={{ color:"hsl(215 25% 48%)" }}>{l}</label>
            <textarea rows={2} value={(form as any)[k]} onChange={f(k)} className="w-full rounded-md px-3 py-2 text-sm resize-none"
              style={{ background:"hsl(216 45% 12%)", color:"hsl(210 40% 88%)", border:"1px solid hsl(216 45% 22%)" }} />
          </div>
        ))}
        <div className="flex gap-2">
          <button onClick={() => onSave(form)} className="flex-1 py-2 rounded-lg text-sm font-semibold"
...
export default function CRMPage() {
  ...
  const filtered = contacts.filter((c) =>
    ...
    (search===""||[c.fullName,c.companyName,c.email].some((v) => v.toLowerCase().includes(search.toLowerCase())))
  );
  ...
  const pipeline = contacts.filter((c) => !["Closed Won","Closed Lost","Active Client"].includes(c.leadStatus));
    ...
    <div className="space-y-5 max-w-7xl">
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          { label:"In Pipeline",    value:pipeline.length,  color:"hsl(38 95% 52%)",  icon:Clock },
          { label:"Active Clients", value:activeClients,    color:"hsl(145 65% 48%)", icon:CheckCircle2 },
          { label:"Closed Won",     value:won,              color:"hsl(217 91% 68%)", icon:Star },
