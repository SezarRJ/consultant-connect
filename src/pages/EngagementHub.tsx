 ...
 * Tabs: Overview | Stakeholders | Tracker | Documents | Activity Log
 * Shows saved outputs from all tools in the timeline.
 */
import { useState } from "react";
import { useEngagementStore, buildEngagementContext, Engagement, EngagementPhase, HealthStatus, Stakeholder } from "@/store/engagementStore";
import { useClaudeAnalysis } from "@/hooks/useClaudeAnalysis";
import {
  Briefcase, Users, Activity, FolderOpen, ClipboardList,
  Plus, Trash2, Edit3, Loader2, FileText, Copy, Check,
  AlertTriangle, ChevronRight, Save, X
} from "lucide-react";

type Tab = "overview"|"stakeholders"|"tracker"|"documents"|"log";
const TABS = [
  { id:"overview" as Tab,      label:"Overview",           icon:Briefcase    },
  { id:"stakeholders" as Tab,  label:"Stakeholders",       icon:Users        },
  { id:"tracker" as Tab,       label:"Tracker",            icon:Activity     },
  { id:"documents" as Tab,     label:"Documents",          icon:FolderOpen   },
  { id:"log" as Tab,           label:"Activity Log",       icon:ClipboardList},
];
...
function NoEngagement() {
  return (
    <div className="flex flex-col items-center justify-center py-20 gap-3">
      ...
      <p className="text-xs" style={{ color:"hsl(215 25% 40%)" }}>Use the banner above or go to CRM → Convert to Engagement.</p>
    </div>
  );
}
...
Respond with a structured engagement brief with these sections:
## Executive Summary
## Client Profile
## Engagement Scope (In Scope / Out of Scope)
## Key Deliverables
## Risk Register
...
## Recommended Next Steps`,
  ...
  const run = () => analyze(`Generate engagement brief. ${extra}`);
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-xl p-4 space-y-3" style={{ background:"hsl(216 45% 10%)", border:"1px solid hsl(216 45% 18%)" }}>
        <p className="text-xs font-semibold" style={{ color:"hsl(210 40% 85%)" }}>Generate Client Briefing Document</p>
        ...
        <div className="flex gap-2">
          ...
          {rawText && (
            <button onClick={() => saveOutput(eng.id, "briefing", "Client Briefing", rawText, "Discovery")}
              className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-semibold"
              style={{ background:"hsl(216 45% 18%)", color:"hsl(38 95% 55%)" }}>
              <Save className="h-4 w-4" /> Save to Engagement
            </button>
          )}
        </div>
      </div>
      ...
      {content && (
        <div className="rounded-xl p-5" style={{ background:"hsl(216 45% 10%)", border:"1px solid hsl(216 45% 18%)" }}>
          <div className="flex items-center justify-between mb-3">
            <p className="text-[10px] uppercase tracking-widest font-semibold" style={{ color:"hsl(215 25% 45%)" }}>Client Brief</p>
...
function OverviewTab({ eng }: { eng: Engagement }) {
  ...
  return (
    <div className="space-y-5">
      {/* Phase tracker */}
      <div className="rounded-xl p-5" style={{ background:"hsl(216 45% 10%)", border:"1px solid hsl(216 45% 18%)" }}>
        <p className="text-[10px] uppercase tracking-widest font-semibold mb-4" style={{ color:"hsl(215 25% 40%)" }}>Engagement Phase</p>
        <div className="flex items-center">
          {PHASES.map((phase, i) => {
            const done = i < phaseIdx; const cur = i === phaseIdx;
            return (
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          { label:"Company",      value:eng.companyName      },
          { label:"Industry",     value:eng.industry         },
          { label:"Market",       value:eng.market           },
          { label:"Service",      value:eng.serviceType      },
          { label:"Budget",       value:eng.budget           },
          { label:"Timeline",     value:eng.timeline         },
          { label:"Priority",     value:eng.priority         },
          { label:"Progress",     value:`${eng.progress}%`   },
        ].map(({ label, value }) => (
          <div key={label} className="rounded-lg p-3" style={{ background:"hsl(216 45% 10%)", border:"1px solid hsl(216 45% 18%)" }}>
            <p className="text-[9px] uppercase tracking-wider font-semibold mb-0.5" style={{ color:"hsl(215 25% 42%)" }}>{label}</p>
      ...
      <div className="flex gap-3">
        <div className="rounded-xl p-4 flex-1" style={{ background:"hsl(216 45% 10%)", border:"1px solid hsl(216 45% 18%)" }}>
          <p className="text-[10px] uppercase tracking-wider font-semibold mb-2" style={{ color:"hsl(215 25% 42%)" }}>Health Status</p>
          <select value={eng.health} onChange={(e) => updateEngagement(eng.id, { health: e.target.value as HealthStatus })}
        ...
        <div className="rounded-xl p-4 flex-1" style={{ background:"hsl(216 45% 10%)", border:"1px solid hsl(216 45% 18%)" }}>
          <p className="text-[10px] uppercase tracking-wider font-semibold mb-2" style={{ color:"hsl(215 25% 42%)" }}>Progress %</p>
          <input type="range" min={0} max={100} value={eng.progress}
            onChange={(e) => updateEngagement(eng.id, { progress: Number(e.target.value) })}
            className="w-full" />
          <p className="text-xs mt-1 text-right font-semibold" style={{ color:"hsl(38 95% 52%)" }}>{eng.progress}%</p>
        </div>
      </div>

      {/* Objectives + risks */}
      {[["Objectives", eng.objectives],["Key Risks", eng.risks],["Scope", eng.scope]].map(([l,v]) => v && (
        <div key={l} className="rounded-xl p-4" style={{ background:"hsl(216 45% 10%)", border:"1px solid hsl(216 45% 18%)" }}>
          <p className="text-[10px] uppercase tracking-wider font-semibold mb-1.5" style={{ color:"hsl(215 25% 42%)" }}>{l}</p>
      ...
      <button onClick={() => setShowBrief((v) => !v)}
        ...
        <FileText className="h-4 w-4" /> {showBrief?"Hide":"Generate"} Client Briefing
      </button>
...
function StakeholdersTab({ eng }: { eng: Engagement }) {
  ...
  const blank: Omit<Stakeholder,"id"> = {
    name:"", position:"", company:eng.companyName, role:"Influencer",
    influence:"Medium", stance:"Neutral", contactDetails:"", notes:"", nextStep:"",
  };
      ...
      {showForm && (
        <div className="rounded-xl p-4 space-y-3" style={{ background:"hsl(216 45% 10%)", border:"1px solid hsl(216 45% 18%)" }}>
          <div className="grid grid-cols-2 gap-3">
            {([["Name","name"],["Position","position"],["Contact","contactDetails"]] as [string,string][]).map(([l,k]) => (
              <div key={k}>
                <label className="block text-[10px] uppercase tracking-wider font-semibold mb-1" style={{ color:"hsl(215 25% 48%)" }}>{l}</label>
                <input type="text" value={(form as any)[k]} onChange={(e) => setForm((p) => ({ ...p,[k]:e.target.value }))}
                  className="w-full rounded-md px-3 py-1.5 text-sm"
                  style={{ background:"hsl(216 45% 14%)", color:"hsl(210 40% 88%)", border:"1px solid hsl(216 45% 22%)" }} />
              </div>
            ))}
            ...
            ] as [string,string,string[]][]).map(([l,k,opts]) => (
              <div key={k}>
                <label className="block text-[10px] uppercase tracking-wider font-semibold mb-1" style={{ color:"hsl(215 25% 48%)" }}>{l}</label>
            ...
            {eng.stakeholders.map((s) => (
              <div key={s.id} className="flex items-center gap-3 px-4 py-3 rounded-xl"
                ...
                <div className="h-9 w-9 rounded-full flex items-center justify-center text-sm font-bold shrink-0"
                  style={{ background:"hsl(216 45% 18%)", color:"hsl(210 40% 72%)" }}>{s.name[0]}</div>
                ...
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full"
                  style={{ color:STANCE_COLOR[s.stance], background:`${STANCE_COLOR[s.stance]}18` }}>{s.stance}</span>
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full"
                  style={{ background:"hsl(216 45% 18%)", color:"hsl(215 25% 55%)" }}>{s.influence}</span>
  ...
  return (
    <div className="space-y-4 grid grid-cols-2 gap-4">
      {[
        ...
        ["Company", "companyName"], ["Market", "market"],
        ["Budget", "budget"], ["Timeline", "timeline"],
        ["Start Date", "startDate"], ["End Date", "endDate"],
      ].map(([label, key]) => (
        <div key={key} className="rounded-lg p-3 col-span-1" style={{ background:"hsl(216 45% 10%)", border:"1px solid hsl(216 45% 18%)" }}>
          <p className="text-[9px] uppercase tracking-wider font-semibold mb-1" style={{ color:"hsl(215 25% 42%)" }}>{label}</p>
          <input type="text" value={(eng as any)[key]||""}
...
function ActivityLogTab({ eng }: { eng: Engagement }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {outputs.length===0
        ? <p className="text-sm text-center py-8" style={{ color:"hsl(215 25% 42%)" }}>No outputs saved yet. Run analysis, strategy, or deliverable tools and save the results.</p>
        : outputs.map((o) => (
          <div key={o.toolId} className="rounded-xl p-4" style={{ background:"hsl(216 45% 10%)", border:"1px solid hsl(216 45% 18%)" }}>
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full"
                  style={{ background:`${PHASE_C[o.phase]||"hsl(215 25% 45%)"}18`, color:PHASE_C[o.phase]||"hsl(215 25% 45%)" }}>
                  {o.phase}
                </span>
    ...
    <div className="space-y-5 max-w-4xl">
      ...
      {!eng ? <NoEngagement /> : (
        <>
          ...
          {tab==="documents"     && (
            <div className="rounded-xl p-8 text-center" style={{ background:"hsl(216 45% 10%)", border:"1px solid hsl(216 45% 18%)" }}>
              <FolderOpen className="h-10 w-10 mx-auto mb-3" style={{ color:"hsl(216 45% 28%)" }} />
              <p className="text-sm" style={{ color:"hsl(215 25% 48%)" }}>Document storage for <strong style={{ color:"hsl(210 40% 82%)" }}>{eng.companyName}</strong>.</p>
              <p className="text-xs mt-1" style={{ color:"hsl(215 25% 38%)" }}>Connect your document backend to load files here.</p>
            </div>
          )}
