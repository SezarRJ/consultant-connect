...
const SUBS = [
  ...
  },
  {
    ...
    description: "Comprehensive engagement report synthesising all findings and recommendations.",
...
function SubRunner({ sub }: { sub: typeof SUBS[0] }) {
  ...
  const [docType, setDocType] = useState(
    sub.id === "proposal" ? PROPOSAL_TYPES[0] : sub.id === "report" ? REPORT_TYPES[0] : ""
  );
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl p-4 space-y-3"
        ...
        {typeOpts && (
          <div>
            <label className="block text-[10px] uppercase tracking-wider font-semibold mb-1.5"
              style={{ color: "hsl(215 25% 48%)" }}>Document Type</label>
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {rawText && (
            <>
              <button onClick={() => { saveOutput(eng.id, sub.id, sub.label, rawText, "Deliverables"); setSaved(true); }}
              ...
              <button onClick={() => { navigator.clipboard.writeText(rawText); setCopied(true); setTimeout(() => setCopied(false), 1500); }}
                className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-semibold"
    ...
    <div className="space-y-5 max-w-4xl">
      <div>
        ...
        <p className="text-sm mt-0.5" style={{ color: "hsl(215 25% 48%)" }}>
          Generate polished client outputs enriched by your saved analysis and strategy.
        </p>
      </div>
