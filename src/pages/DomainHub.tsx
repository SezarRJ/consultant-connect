 ...
 * Route param :domain maps to one of 8 sector tools
 */
...
const DOMAINS: Record<string, {
  ...
  description:string; tier:"flash-lite"|"flash"|"pro";
  subTools:{id:string; label:string; prompt:string; fields?:{key:string;label:string;placeholder:string}[]}[];
}> = {
  "real-estate": {
    ...
    tier:"flash",
    subTools:[
      { id:"location",    label:"Location Intelligence",   prompt:"Analyze this real estate location — catchment, demographics, access, competition, and development potential.",
  ...
  "fmcg": {
    ...
    tier:"flash",
    subTools:[
      { id:"rtm",     label:"Route to Market Design", prompt:"Design a full RTM strategy for this FMCG engagement — channels, coverage model, distributor structure, KPIs.",
  ...
  "fnb": {
    ...
    subTools:[
      ...
      { id:"menu",    label:"Menu Strategy",        prompt:"Menu strategy — localization, pricing, margin optimization for this market." },
    ],
  },
  "telecom": {
  ...
  "distribution": {
    ...
    description:"Supply chain design, logistics optimization, and distribution network strategy.",
    tier:"flash",
    subTools:[
      ...
      { id:"logistics",label:"Logistics Optimization",prompt:"Logistics optimization — routing, lead times, 3PL vs own fleet, and KPIs." },
...
function DomainTool({ domain, toolId }: { domain: typeof DOMAINS[string]; toolId: string }) {
  const { getActiveEngagement, saveOutput } = useEngagementStore();
  const eng = getActiveEngagement();
  const tool = domain.subTools.find((t) => t.id===toolId)!;
  const [fields, setFields] = useState<Record<string,string>>({});
  const [extra, setExtra] = useState("");
  const [saved, setSaved] = useState(false);

  const { analyze, loading, rawText } = useClaudeAnalysis({
    modelTier: domain.tier,
    systemPrompt: `You are a senior ${domain.label} consultant for MENA markets with deep Iraq expertise.
...
Provide a structured, professional ${tool.label} analysis with clear sections and actionable recommendations.`,
  ...
  const handleSave = () => {
    if (rawText && eng) {
      saveOutput(eng.id, `${domain.label}-${toolId}`, `${domain.label}: ${tool.label}`, rawText, "Analysis");
      setSaved(true);
    }
  };
    ...
    <div className="space-y-4">
      {!eng && (
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg text-xs"
          ...
          <AlertTriangle className="h-3.5 w-3.5" /> No active engagement — results won't be saved automatically.
        </div>
      )}
      {eng && (
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg text-xs"
      ...
      <div className="rounded-xl p-4 space-y-3" style={{ background:"hsl(216 45% 10%)", border:"1px solid hsl(216 45% 18%)" }}>
        {tool.fields?.map((f) => (
          <div key={f.key}>
            ...
            <input type="text" placeholder={f.placeholder} value={fields[f.key]??""} onChange={(e) => setFields((p) => ({ ...p,[f.key]:e.target.value }))}
        ...
        <div>
          ...
          <textarea rows={2} value={extra} onChange={(e) => setExtra(e.target.value)}
            placeholder="Specific focus areas or questions…" className="w-full rounded-md px-3 py-2 text-sm resize-none"
            style={{ background:"hsl(216 45% 14%)", color:"hsl(210 40% 88%)", border:"1px solid hsl(216 45% 22%)" }} />
        </div>
        <div className="flex gap-2">
          <button onClick={run} disabled={loading}
            className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold disabled:opacity-50"
            style={{ background:domain.color, color:"hsl(216 58% 6%)" }}>
            {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <domain.icon className="h-4 w-4" />}
            Run {tool.label}
          </button>
          {rawText && eng && (
      ...
      {rawText && (
        <div className="rounded-xl p-5" style={{ background:"hsl(216 45% 10%)", border:"1px solid hsl(216 45% 18%)" }}>
          <p className="text-[10px] uppercase tracking-widest font-semibold mb-3" style={{ color:"hsl(215 25% 45%)" }}>{tool.label}</p>
          <div className="text-sm leading-relaxed whitespace-pre-wrap" style={{ color:"hsl(210 40% 80%)" }}>{rawText}</div>
        </div>
      )}
    </div>
  );
    ...
    <div className="space-y-5 max-w-4xl">
      ...
      <div className="flex items-center gap-3 px-4 py-3 rounded-xl"
        ...
        <div>
          ...
          <p className="text-xs" style={{ color:"hsl(215 25% 48%)" }}>{tool.prompt.slice(0,80)}…</p>
        </div>
      </div>

      <DomainTool domain={cfg} toolId={tool.id} />
    </div>
  );
}
