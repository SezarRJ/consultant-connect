...
const BLANK_ENTRY: Omit<BillableEntry, "id"> = {
  ...
  activity: "", consultant: "", billable: true
};

function load(): Engagement[] {
  try { const d = JSON.parse(localStorage.getItem(LS_KEY) || "null"); return d || []; }
  catch { return []; }
}
                ...
                <div className="mb-4 p-4 rounded-xl space-y-3" style={{ background: "hsl(216 45% 10%)", border: "1px solid hsl(var(--border))" }}>
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <input value={logForm.consultant} onChange={e => setLogForm(f => ({ ...f, consultant: e.target.value }))}
                        placeholder="Name or role" className="w-full px-3 py-2 rounded-lg text-sm" style={IS} />
                    </div>
