...
import { Lightbulb, BarChart2, Zap, Handshake, BookOpen, Loader2, AlertTriangle, Save, Check } from "lucide-react";
...
const FRAMEWORKS = ["SWOT Analysis","Porter's Five Forces","Ansoff Matrix","BCG Matrix","PESTLE Analysis","VRIO Framework"];
...
function SubRunner({ sub }: { sub: typeof SUBS[0] }) {
  ...
  const { analyze, loading, rawText } = useClaudeAnalysis({
    modelTier: sub.tier,
    systemPrompt: `You are a senior management consultant specializing in ${sub.label}.
...
## Suggested Next Step`,
  ...
  const handleSave = () => {
    if (rawText) { saveOutput(eng.id, sub.id, sub.label, rawText, "Strategy"); setSaved(true); }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl p-4 space-y-3" style={{ background:"hsl(216 45% 10%)", border:"1px solid hsl(216 45% 18%)" }}>
        {sub.id==="workshop" && (
          <div>
            <label className="block text-[10px] uppercase tracking-wider font-semibold mb-1.5" style={{ color:"hsl(215 25% 48%)" }}>Framework</label>
