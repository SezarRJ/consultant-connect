...
function SubRunner({ sub }: { sub: Sub }) {
  ...
  const { analyze, loading, rawText } = useClaudeAnalysis({
    modelTier: sub.tier,
    systemPrompt: `You are a senior management consultant specialising in ${sub.label} for the MENA region with deep Iraq expertise.
${buildFullContext(eng, sub.priorTools)}
Provide a structured, professional ${sub.label} report with clear sections.
Format:
## Summary
## Key Findings
## Risks & Opportunities
...
## Suggested Next Action`,
  ...
  const handleSave = () => {
    if (rawText) { saveOutput(eng.id, sub.id, sub.label, rawText, "Analysis"); setSaved(true); }
  };
