/**
 * Dashboard.tsx — engagement-centered home
 * Practice Ops section is below the main workspace cards
 */
...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-7 max-w-6xl mx-auto">
      ...
      {/* Active Engagement */}
      {active ? (
        <div className="rounded-xl p-5" style={{ background:"hsl(216 45% 10%)", border:`1px solid ${PHASE_COLOR[active.phase]}35` }}>
          <div className="flex items-start justify-between gap-4 mb-4">
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full"
                  style={{ background:`${PHASE_COLOR[active.phase]}18`, color:PHASE_COLOR[active.phase] }}>{active.phase}</span>
                <span className="text-[10px] px-2 py-0.5 rounded-full" style={{ background:"hsl(216 45% 18%)", color:"hsl(215 25% 55%)" }}>{active.status}</span>
              </div>
              <h2 className="text-lg font-bold" style={{ color:"hsl(210 40% 92%)" }}>{active.companyName||active.clientName}</h2>
              <p className="text-sm" style={{ color:"hsl(215 25% 52%)" }}>{active.serviceType} · {active.market} · {active.industry}</p>
            </div>
            <Link to="/engagement" className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold shrink-0"
              style={{ background:"hsl(38 95% 52%)", color:"hsl(216 58% 6%)" }}>
          ...
          {Object.keys(active.outputs).length > 0 && (
            <div className="flex items-center gap-2 mb-3 text-xs" style={{ color:"hsl(145 65% 48%)" }}>
              ...
              {Object.keys(active.outputs).length} saved output{Object.keys(active.outputs).length>1?"s":""}
              · chain active
            </div>
          )}
        ...
        <div className="rounded-xl p-5 flex items-center gap-4"
          ...
          <div className="flex-1">
            ...
            <p className="text-xs mt-0.5" style={{ color:"hsl(215 25% 48%)" }}>Start in CRM → qualify a lead → convert to engagement.</p>
          </div>
          <Link to="/crm" className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold shrink-0"
            style={{ background:"hsl(38 95% 52%)", color:"hsl(216 58% 6%)" }}>
        ...
        <div className="flex items-stretch gap-0">
          {WORKFLOW.map((w, i) => (
            <div key={w.url} className="flex items-center flex-1">
              ...
              {i < WORKFLOW.length-1 && <ChevronRight className="h-4 w-4 shrink-0 mx-1" style={{ color:"hsl(216 45% 25%)" }} />}
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2">
          <p className="text-[10px] uppercase tracking-widest font-semibold mb-3" style={{ color:"hsl(215 25% 38%)" }}>INDUSTRY DOMAINS</p>
          <div className="grid grid-cols-4 gap-2">
            {DOMAINS.map((d) => (
              <Link key={d.url} to={d.url}
                className="group flex flex-col items-center gap-2 p-3 rounded-xl text-center transition-all hover:scale-[1.03]"
              ...
              {engagements.slice(0,5).map((e) => (
                <button key={e.id} onClick={() => setActiveEngagement(e.id)}
                  ...
                  <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full shrink-0"
                    style={{ background:`${PHASE_COLOR[e.phase]}18`, color:PHASE_COLOR[e.phase] }}>{e.phase}</span>
                </button>
              ))}
            </div>
          )}
