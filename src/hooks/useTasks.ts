...
export function useTasks() {
  return useQuery({
    queryKey: ["tasks"],
    queryFn: async () => {
      const { data, error } = await (supabase as any).from("tasks").select("*").order("due_date",{ascending:true});
...
export function useUpdateTask() {
  ...
  return useMutation({
    mutationFn: async ({ id, ...t }: Partial<Task> & { id:string }) => {
      ...
      const { data, error } = await (supabase as any).from("tasks").update(patch).eq("id",id).select().single();
      if (error) throw error;
      return fromRow(data);
    },
    onSuccess: () => qc.invalidateQueries({ queryKey:["tasks"] }),
